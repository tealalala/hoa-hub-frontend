<template>
  <div class="votesCCRs">
    <div class="container">
      <h1>Vote on Proposed CC&Rs</h1>

      <div class="alert alert-secondary" role="alert">
        <h6>CC&Rs - The Community's Standards</h6>
        ...
        <a href="/#/ccrs">View the approved CC&Rs</a>
      </div>
      ...
      <table width="100%" border=1 cellpadding=10 cellspacing=10>
        ...
        <tbody>
          <tr v-for="action in filtered_ccrs_is_false">
            <td>{{ action.id }}</td>
            <td>{{ action.status }}</td>
...
export default {
  data: function() {
    return {
      user: [],
      filtered_ccrs_is_false: [],
      errors: []
    };
  },
  created: function() {
    axios.get('http://localhost:3000/api/filtered_ccrs_is_false').then(function(response) {
      this.filtered_ccrs_is_false = response.data;
    }.bind(this))
  },
  methods: {
    // fetchUser: function() {
    ...
    voteUp: function(inputAction) {
      ...
      axios
        ...
        .then(response => {
          ...
          // console.log(inputAction);
          var index = this.filtered_ccrs_is_false.indexOf(inputAction);
          this.filtered_ccrs_is_false.splice(index, 1);
          this.errors = [];
        })
        .catch(error => {
